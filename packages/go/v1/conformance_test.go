...
func TestEncodeDecodeConformance(t *testing.T) {
	...
	for _, c := range fixture.Cases {
		t.Run(c.ID, func(t *testing.T) {
			fields := v1.Fields{Timestamp: uintValue(t, c.Timestamp), Type: c.Type, Node: c.Node, Sequence: c.Sequence}
			id, err := v1.Encode(fields)
			if err != nil {
				t.Fatal(err)
			}
			if got := v1.ToDecimalString(id); got != c.IDDecimal {
...
func TestDecimalRejectConformance(t *testing.T) {
	...
	for _, c := range fixture.Cases {
		t.Run(c.ID, func(t *testing.T) {
			_, err := v1.FromDecimalString(c.Input)
			var orbitErr *v1.Error
			if !errors.As(err, &orbitErr) || orbitErr.Code != v1.InvalidDecimal {
				t.Fatalf("error = %v, want INVALID_DECIMAL", err)
			}
			if _, err := v1.Parse(c.Input); err == nil || v1.IsValid(c.Input) {
				t.Fatal("invalid decimal accepted")
			}
		})
	}
	if id, err := v1.FromDecimalString("0"); err != nil || id != 0 || !v1.IsValid("0") {
...
func TestGeneratorConformance(t *testing.T) {
	...
	for _, c := range fixture.Cases {
		t.Run(c.ID, func(t *testing.T) {
			generator, err := v1.NewGenerator(v1.GeneratorOptions{
				Node: c.Node, Clock: fixedClock{now: int64(uintValue(t, c.NowTimestamp))},
				ClockRollbackToleranceMs: tolerance, OnSequenceExhausted: v1.SequenceExhaustedFail,
			})
			if err != nil {
				t.Fatal(err)
			}
			...
			case "issue":
				if decision.Action != v1.DecisionIssue || decision.Timestamp != uintValue(t, c.Expect.Timestamp) || decision.Sequence != c.Expect.Sequence {
					t.Fatalf("decision = %#v", decision)
				}
			case "wait":
				if decision.Action != v1.DecisionWait || decision.WaitUntilTimestamp != uintValue(t, c.Expect.WaitUntilTimestamp) {
					t.Fatalf("decision = %#v", decision)
				}
			case "wait_or_fail":
				if decision.Action != v1.DecisionError || decision.Error != v1.SequenceExhausted {
					t.Fatalf("decision = %#v", decision)
				}
			case "error":
				if decision.Action != v1.DecisionError || string(decision.Error) != c.Expect.Error {
					t.Fatalf("decision = %#v", decision)
				}
			}
		})
...
func TestGeneratorWaitModeAndReservedType(t *testing.T) {
	generator, err := v1.NewGenerator(v1.GeneratorOptions{
		Node: 7, Clock: fixedClock{now: 1000}, OnSequenceExhausted: v1.SequenceExhaustedWait,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if decision.Action != v1.DecisionWaitNextMs || decision.FromTimestamp != 1000 {
		t.Fatalf("decision = %#v", decision)
	}
...
func TestGeneratorGenerateAndHelpers(t *testing.T) {
	...
	generator, err := v1.NewGenerator(v1.GeneratorOptions{
		Node: 7, Clock: clock, OnSequenceExhausted: v1.SequenceExhaustedWait,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	waiter, err := v1.NewGenerator(v1.GeneratorOptions{
		Node: 7, Clock: waitClock, OnSequenceExhausted: v1.SequenceExhaustedWait,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := v1.Encode(v1.Fields{Timestamp: v1.MaxTimestamp + 1, Type: 1, Node: 1, Sequence: 0}); err == nil {
		t.Fatal("expected timestamp overflow")
	}
	if _, err := v1.Encode(v1.Fields{Timestamp: 1, Type: 99, Node: 1, Sequence: 0}); err == nil {
		t.Fatal("expected type overflow")
	}
	if _, err := v1.Encode(v1.Fields{Timestamp: 1, Type: 1, Node: 999, Sequence: 0}); err == nil {
		t.Fatal("expected node overflow")
	}
	if _, err := v1.Encode(v1.Fields{Timestamp: 1, Type: 1, Node: 1, Sequence: 9999}); err == nil {
		t.Fatal("expected sequence overflow")
	}

	unix := v1.ToUnixTimeMs(0)
	if v1.FromUnixTimeMs(int64(unix)) != 0 {
		t.Fatal("unix roundtrip failed")
	}
	_ = v1.SystemClock().CurrentOrbitTimestampMs()

	lost, err := v1.NewGenerator(v1.GeneratorOptions{
