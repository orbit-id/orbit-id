...
export type GenerateDecisionV2 =
...
export class OrbitGeneratorV2 {
  ...
  constructor(options: GeneratorOptionsV2) {
    const { node } = options;
  ...
  decide(type: number, nowTimestamp?: bigint): GenerateDecisionV2 {
  ...
  private generateUnlocked(type: number): bigint {
    for (;;) {
      ...
      switch (decision.action) {
        case "issue": {
          const id = encode({
            formatVersion: ISSUED_FORMAT_VERSION,
